...
def my_sample(model, gen_data_dir, sample_batch_size = 25, obs = (3,32,32), sample_op = sample_op):
    for label in my_bidict:
        print(f"Label: {label}")
        #generate images for each label, each label has 25 images, modify sample function to take in label input
        sample_t = sample(model, torch.tensor([my_bidict[label]]).expand(sample_batch_size).to(device), sample_batch_size, obs, sample_op)
